...
from episode_runner_subgoal_sequential import EpisodeRunnerSubgoalSequential
from model_saver import ModelSaver
from network_subgoal_sequential import Network
...
from trainer_subgoal_sequential import TrainerSubgoalSequential
...
def run_for_config(config):
    ...
    with tf.compat.v1.Session(
        ...
        episode_runner = EpisodeRunnerSubgoalSequential(config, game, policy_function)
        trainer = TrainerSubgoalSequential(
        ...
        for cycle in range(config['general']['training_cycles_per_level']):
            ...
            new_global_step, success_ratio = trainer.train_policy_at_level(global_step)
            ...
            if (cycle + 1) % config['policy']['decrease_std_every'] == 0:
                ...
                print_and_log('new base stds {}'.format(network.get_base_stds(sess)))
            ...
            if cycle % config['general']['test_frequency'] == 0:
                # do test
                test_successes, test_cost, _, endpoints_by_path = trainer.collect_test_data(False)
                ...
                with open(episodic_success_rates_path, 'a') as f:
                    f.write('{} {} {} {}'.format(
                        trainer.train_episodes_counter, test_successes, test_cost, os.linesep))

                # decide how to act next
                print_and_log('old cost was {} at step {}'.format(best_cost, best_cost_global_step))
                print_and_log('current learn rates {}'.format(network.get_learn_rates(sess)))
                print_and_log('current base stds {}'.format(network.get_base_stds(sess)))
                if best_cost is None or test_cost < best_cost:
                    ...
                    if no_test_improvement == decrease_learn_rate_if_static_success:
                        ...
                        print_and_log('new learn rates {}'.format(network.get_learn_rates(sess)))
